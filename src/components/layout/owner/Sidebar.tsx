...
import { LayoutDashboard, Building2, Inbox, Star, Settings, Bell, LogOut } from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { name: 'Inquiries', path: '/owner/inquiries', icon: Inbox },
...
export function SidebarOwner() {
  ...
  return (
    <aside className="fixed top-0 left-0 w-full h-14 md:relative md:w-[72px] md:h-full flex-shrink-0 flex flex-row md:flex-col items-center justify-between md:justify-start px-2 md:px-0 py-0 md:py-6 border-b md:border-b-0 md:border-r border-border/50 bg-card z-50 shadow-[0_4px_24px_rgba(0,0,0,0.05)] md:shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      <Link to="/owner" className="hidden md:flex w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-primary/80 text-primary-foreground items-center justify-center font-black text-lg mb-8 shadow-lg shadow-primary/20 hover:scale-105 transition-transform">
        B
      </Link>
      ...
      <nav className="flex flex-row md:flex-col items-center gap-1 md:gap-3 w-auto md:w-full px-1 md:px-3">
        {nav.map((item) => {
          const isActive = location.pathname === item.path || 
                           (location.pathname.startsWith(item.path) && item.path !== '/owner');
          const Icon = item.icon;
          const hasBadge = item.name === 'Inquiries' && unreadInquiriesCount > 0;
      ...
      <div className="md:mt-auto flex flex-row md:flex-col items-center gap-1 md:gap-3 w-auto md:w-full px-1 md:px-3 pt-0 md:pt-6 border-t-0 md:border-t border-border/50 relative">
        ...
        <Link to="/owner/settings" title="Settings" className="hidden md:flex flex items-center justify-center w-12 h-12 text-muted-foreground hover:text-foreground hover:bg-muted rounded-xl transition-colors">
        ...
        <div className="flex flex items-center justify-center w-12 h-12 md:mt-2">
          <Link to="/owner/settings">
            <img 
              src={user?.image ?? `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name ?? 'Owner')}&background=000&color=fff&size=40`} 
              alt="Owner Avatar" 
