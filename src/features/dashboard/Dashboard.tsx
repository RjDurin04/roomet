...
const MAX_TOP_RATED = 4;
const MAX_INQUIRIES = 4;
const MAX_REVIEWS = 3;

// eslint-disable-next-line max-lines-per-function, complexity -- Dashboard integrates multiple queries
export function Dashboard() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      void navigate(`/tenant/map?q=${encodeURIComponent(searchQuery.trim())}`);
    } else {
      void navigate('/tenant/map');
    }
  };
  ...
  const topRated = properties.slice(0, MAX_TOP_RATED);
  ...
  const stats = [
    ...
    { label: 'My Reviews', value: reviews.length, icon: PenLine, path: '#' },
  ];
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-10 py-6 md:py-8 space-y-6 md:space-y-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:items-end justify-between">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 lg:pb-0">
            {stats.map((s: any) => (
              ...
              >
                <div className="w-8 h-8 rounded-xl bg-muted flex items-center justify-center group-hover:bg-primary/10 transition-colors shrink-0">
        ...
        <div className="relative">
          <form onSubmit={handleSearch} className="relative z-10">
            ...
            />
            <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 bg-primary text-primary-foreground h-10 px-4 md:px-6 rounded-xl text-[13px] font-bold hover:bg-primary/90 transition-colors">
              Find Stays
            </button>
          </form>
            ...
            {featured ? (
              ...
              >
                ...
                <div className="absolute top-4 md:top-6 left-4 md:left-6 flex gap-2">
                  <span className="bg-white/10 backdrop-blur-md border border-white/20 text-white text-[9px] md:text-[10px] font-bold uppercase tracking-widest px-2.5 md:px-3 py-1 md:py-1.5 rounded-full">
                    Verified
                  </span>
                  <span className="bg-amber-500 text-white text-[9px] md:text-[10px] font-bold uppercase tracking-widest px-2.5 md:px-3 py-1 md:py-1.5 rounded-full">
                    Top Recommended
                  </span>
                </div>

                <div className="absolute inset-0 p-5 md:p-8 flex flex-col justify-end w-full">
                  <h3 className="text-2xl md:text-3xl font-extrabold text-white mb-1 md:mb-2">{featured.name}</h3>
                  <p className="text-white/70 text-[13px] md:text-[14px] flex items-center gap-1.5 mb-4 md:mb-5 truncate">
                    <MapPin className="w-4 h-4 shrink-0" /> {featured.location.address}
                  </p>
                  <div className="flex items-center gap-3 sm:gap-6 justify-between sm:justify-start w-full">
                    <div>
                    ...
                    <button className="sm:ml-auto w-10 h-10 md:w-12 md:h-12 bg-white text-black rounded-full flex items-center justify-center hover:bg-primary hover:text-white transition-all transform group-hover:rotate-[-45deg] shrink-0">
              ...
              <div className="rounded-[2rem] border border-border/50 bg-card/50 flex flex-col items-center justify-center h-[280px] md:h-[340px] text-muted-foreground p-8 text-center space-y-4 shadow-sm">
                ...
                <button onClick={() => { void navigate('/tenant/map'); }} className="text-[12px] font-bold text-primary hover:underline">Go to Map</button>
              </div>
            )}
          </div>
              ...
              <AnimatePresence mode="wait">
                {activeTab === 'inquiries' ? (
                  <motion.div key="inq" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-1">
                    ...
                    {inquiries.length > 0 && (
                      <button onClick={() => { void navigate('/tenant/inquiries'); }} className="w-full py-4 text-[11px] font-bold text-muted-foreground hover:text-primary transition-colors">
                  ...
                  <motion.div key="rev" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-3 p-2">
                    {reviews.length > 0 ? reviews.slice(0, MAX_REVIEWS).map((rev: unknown) => {
                      ...
                      <div 
                        key={r._id} 
                        onClick={() => { void navigate(`/tenant/map/roomet/${r.propertyId}?tab=reviews&reviewId=${r._id}`); }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-card border border-border rounded-[2rem] overflow-hidden p-6 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { void navigate('/tenant/map'); }} className="text-[11px] font-bold text-muted-foreground hover:text-foreground">Browse all</button>
            </div>
            {topRated.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {topRated.map((bh: unknown) => {
                  ...
                  <div 
                    key={b._id} 
                    onClick={() => { void navigate(`/tenant/map/roomet/${b._id}`); }}
          ...
          <div className="bg-primary text-primary-foreground rounded-[2rem] p-8 flex flex-col justify-between relative overflow-hidden group">
            <div className="relative z-10 h-full flex flex-col justify-center">
              ...
              <button 
                onClick={() => { void navigate('/tenant/map'); }}
