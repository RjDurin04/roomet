...
import { User, Mail, Shield, Moon, Sun, LogOut, Camera, Check, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export function ProfileSettings() {
  ...
  React.useEffect(() => {
    ...
    async function checkAccount() {
      try {
        ...
        // RES-C02: Fail open — don't assume "no credential account" on fetch failure.
        // Showing the password field is safer than hiding it.
        console.error("[ProfileSettings] Failed to check account type:", e);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('[ProfileSettings] Image upload failed:', error);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // RES-C01: Surface profile save failures to the user
      const message = error instanceof Error ? error.message : 'Failed to save profile';
      setProfileSuccess('');
      alert(message);
      console.error('[ProfileSettings] Profile update failed:', message);
    } finally {
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      if (!hasCredentialAccount) {
        ...
        if (error) {
          // RES-H01: Use error code, not string matching
          ...
          <div className="flex items-center gap-5">
            <div className="relative group">
              {isUploading ? (
                ...
                <img 
                  src={user?.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'User')}&background=1a1a1a&color=fff&size=80&font-size=0.35`} 
                  alt="Avatar" 
        ...
        <section className="bg-card border border-border rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-border">
            <h3 className="text-[14px] font-bold flex items-center gap-2">
              {theme === 'dark' ? <Moon className="w-4 h-4 text-primary" /> : <Sun className="w-4 h-4 text-primary" />} Preferences
            </h3>
          </div>
