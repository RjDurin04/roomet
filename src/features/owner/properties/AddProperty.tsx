...
function UnitTypeSelect({ value, onChange, isError }: { value: string, onChange: (val: string) => void, isError?: boolean }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={!value ? (isError ? 'text-rose-500/70' : 'text-foreground/50 dark:text-muted-foreground/30') : ''}>{value || 'Select Type'}</span>
        <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''} ${isError ? 'text-rose-500' : 'text-foreground/40 dark:text-muted-foreground/50'}`} />
      </button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute top-full left-[-16px] right-[-16px] mt-2 bg-card/90 border border-border dark:border-border/50 rounded-2xl shadow-2xl z-50 overflow-hidden py-2 backdrop-blur-xl"
            >
              {options.map((option) => (
                <button
                  key={option}
                  ...
                  className={`w-full px-5 py-2.5 text-left text-[13px] font-bold transition-colors hover:bg-primary/10 hover:text-primary ${value === option ? 'text-primary bg-primary/10 dark:bg-primary/5' : 'text-muted-foreground'}`}
                >
                  {option}
                </button>
              ))}
...
// RES-L03: Typed room data instead of any
...
function RoomUnitCard({
  ...
  return (
    <motion.div
      ...
      className="bg-card border border-border dark:border-border/50 rounded-[24px] p-5 group hover:border-primary/40 transition-all shadow-sm hover:shadow-md relative overflow-hidden"
    >
      ...
      <div className="flex flex-col md:flex-row gap-5">
        <div className="flex-1 space-y-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-muted/80 dark:bg-muted/50 flex items-center justify-center text-muted-foreground group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-500 shadow-inner">
              <BedDouble className="w-5 h-5" />
            </div>
            <div className="flex gap-4 flex-1 min-w-0">
              <div className="flex-1">
                <label className={`text-[9px] font-black uppercase tracking-widest mb-1 block transition-colors ${errors[`room-${idx}-name`] ? 'text-rose-500' : 'text-foreground/50 dark:text-muted-foreground'}`}>Unit Name</label>
                <input
                  type="text"
                  value={room.name}
                  onChange={(e) => { onUpdate(idx, { name: e.target.value }); }}
                  placeholder="e.g. Room 1"
                  className={`w-full bg-transparent border-0 outline-none p-0 text-lg font-black transition-all placeholder:text-foreground/30 dark:placeholder:text-muted-foreground/50 focus:outline-none focus:ring-0 shadow-none appearance-none ${errors[`room-${idx}-name`] ? 'text-rose-500 placeholder:text-rose-300/40' : 'text-foreground'}`}
                />
              </div>
              <div className="flex-1 relative">
                <label className={`text-[9px] font-black uppercase tracking-widest mb-1 block transition-colors ${errors[`room-${idx}-type`] ? 'text-rose-500' : 'text-foreground/50 dark:text-muted-foreground'}`}>Unit Type</label>
                <UnitTypeSelect
                  value={room.type}
          ...
          <div className="flex flex-wrap items-center gap-6 pt-3 border-t border-border dark:border-border/50 dark:border-border/30">
            <div className="flex flex-col gap-1.5 min-w-[140px]">
              <label className="text-[9px] font-black uppercase tracking-widest text-foreground/50 dark:text-muted-foreground mb-1 block">Gender Policy</label>
              <div className="flex bg-muted/60 dark:bg-muted/30 p-1 rounded-xl border border-border dark:border-border/50 h-10">
                {[
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 px-2 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${room.gender === policy.id ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted/80 dark:bg-muted/50'}`}
                  >
            ...
            <div className="flex gap-4">
              <div className="flex flex-col gap-1.5 min-w-[100px]">
                <label className="text-[9px] font-black uppercase tracking-widest text-foreground/50 dark:text-muted-foreground">Max Capacity</label>
                <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-muted/60 dark:bg-muted/30 border border-border dark:border-border/50 shadow-inner h-10 group/cap">
                  <button
                    type="button"
                    disabled={room.type === 'Single' || (room.type === 'Shared' && room.capacity <= 2)}
                    onClick={() => { onUpdate(idx, { capacity: room.capacity - 1 }); }}
                    className="w-6 h-6 rounded-lg bg-background border border-border dark:border-border/50 flex items-center justify-center hover:bg-muted disabled:opacity-30 disabled:cursor-not-allowed transition-colors flex-shrink-0"
                  >
                    <Minus className="w-3 h-3" />
                  </button>
                  <span className={`text-sm font-black tabular-nums ${room.type === 'Single' ? 'text-muted-foreground/80 dark:text-muted-foreground/50' : ''}`}>{room.capacity}</span>
                  <button
                    type="button"
              ...
              <div className="flex flex-col gap-1.5 min-w-[100px]">
                <label className="text-[9px] font-black uppercase tracking-widest text-foreground/50 dark:text-muted-foreground">Occupied</label>
                <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-muted/60 dark:bg-muted/30 border border-border dark:border-border/50 shadow-inner h-10 group/occ">
                  <button
                    type="button"
        ...
        <div className="w-full md:w-[240px] shrink-0 bg-muted/80 dark:bg-muted/50 dark:bg-muted/20 rounded-[20px] p-4 border border-border dark:border-border/50 dark:border-border/30 space-y-4 flex flex-col justify-center">
          <div>
            <label className="block text-[9px] font-black uppercase tracking-widest text-foreground/50 dark:text-muted-foreground mb-2.5">Pricing Model</label>
            <div className="flex bg-background p-1 rounded-xl border border-border/40 dark:border-border/20 shadow-inner">
              <button
                onClick={() => { onUpdate(idx, { priceType: 'person' }); }}
                className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase tracking-tighter transition-all ${room.priceType === 'person' ? 'bg-primary text-primary-foreground shadow-md' : 'text-muted-foreground hover:bg-muted/80 dark:bg-muted/50'}`}
              >
                Per Person
              </button>
              <button
                onClick={() => { onUpdate(idx, { priceType: 'room' }); }}
                className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase tracking-tighter transition-all ${room.priceType === 'room' ? 'bg-primary text-primary-foreground shadow-md' : 'text-muted-foreground hover:bg-muted/80 dark:bg-muted/50'}`}
              >
                Whole Unit
              </button>
            </div>
          ...
          <div className="pt-1">
            <label className={`block text-[9px] font-black uppercase tracking-widest mb-1 transition-colors ${errors[`room-${idx}-price`] ? 'text-rose-500' : 'text-foreground/50 dark:text-muted-foreground'}`}>Monthly Rent</label>
            <div className="flex items-baseline gap-2">
              ...
              <input
                ...
                className={`w-full h-12 bg-transparent border-b-2 outline-none text-2xl font-black tabular-nums tracking-tighter transition-colors ${errors[`room-${idx}-price`]
                    ...
                    : 'border-border focus:border-primary placeholder:text-muted-foreground/70 dark:text-muted-foreground/40'
                  }`}
              />
              <span className={`text-[12px] font-black uppercase tracking-widest mb-1.5 shrink-0 whitespace-nowrap transition-colors ${errors[`room-${idx}-price`] ? 'text-rose-400/50' : 'text-primary/70 dark:text-primary/40'}`}>/ {room.priceType === 'person' ? 'Head' : 'Unit'}</span>
            </div>
      ...
      <div className="mt-4 pt-4 border-t border-border dark:border-border/50 dark:border-border/30">
        <div className="flex flex-col gap-1 mb-3">
          <label className="block text-[9px] font-black uppercase tracking-widest text-foreground/50 dark:text-muted-foreground">Unit-Specific Features</label>
        </div>
        <div className="flex gap-2 mb-3">
          <input
            ...
            className="flex-1 bg-background border border-border dark:border-border/60 rounded-xl px-3 py-1.5 text-[11px] outline-none focus:border-primary transition-colors placeholder:text-muted-foreground/80 dark:text-muted-foreground/50"
          />
          <button
            onClick={addCustom}
          ...
          <AnimatePresence>
            {room.amenities?.map((custom: string) => (
              <motion.div
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-primary/30 bg-primary/10 dark:bg-primary/5 text-primary text-[10px] font-black uppercase tracking-tight"
              >
                {custom}
...
export function AddProperty() {
  ...
  const [formData, setFormData] = useState({
    ...
    rooms: [
      { id: crypto.randomUUID(), name: '', type: '', gender: 'mixed', capacity: 1, occupied: 0, price: 0, priceType: 'person' as const, amenities: [] as string[] }
    ] as RoomFormData[],
    amenities: [] as string[],
    rules: '',
  });
    ...
    switch (currentStep) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
              <div className="space-y-6">
                ...
                <div className="relative group">
                  <MapPin className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 ${errors.address ? 'text-red-500' : 'text-primary'}`} />
                  <input
                    type="text"
                    value={formData.address}
                    onChange={e => { setFormData({ ...formData, address: e.target.value }); }}
                    placeholder="Street address"
                    className="w-full h-12 pl-12 pr-4 bg-background border border-border dark:border-border/60 focus:border-primary rounded-xl text-[13px] outline-none transition-colors placeholder:text-foreground/40 dark:placeholder:text-muted-foreground/50"
                  />
                </div>
                {errors.address && <p className="text-red-500 text-[10px] font-bold mt-1.5 ml-1">{errors.address}</p>}

                <div className="grid grid-cols-2 gap-4 mt-2">
                  <div className="p-4 rounded-[16px] bg-muted/60 dark:bg-muted/30 border border-border dark:border-border/50 flex flex-col gap-1 shadow-inner group hover:bg-muted/40 transition-colors">
                    <span className="text-[9px] font-black text-primary/80 dark:text-primary/40 uppercase tracking-[0.2em]">Latitude</span>
                    <span className="font-bold text-sm tracking-tight text-foreground/90">{formData.mapPin.lat.toFixed(6)}</span>
                  </div>
                  <div className="p-4 rounded-[16px] bg-muted/60 dark:bg-muted/30 border border-border dark:border-border/50 flex flex-col gap-1 shadow-inner group hover:bg-muted/40 transition-colors">
                    <span className="text-[9px] font-black text-primary/80 dark:text-primary/40 uppercase tracking-[0.2em]">Longitude</span>
                    <span className="font-bold text-sm tracking-tight text-foreground/80">{formData.mapPin.lng.toFixed(6)}</span>
                  </div>
                </div>
              </div>
              <div className="h-[400px] rounded-[32px] overflow-hidden border border-border dark:border-border/50 bg-background relative shadow-inner lg:sticky lg:top-8">
                <Map
        ...
        return (
          <div className="space-y-8">
            <div className="flex justify-between items-end mb-4">
              ...
              <button
                onClick={() => { setFormData({ ...formData, rooms: [...formData.rooms, { id: crypto.randomUUID(), name: '', type: '', gender: 'mixed', capacity: 1, occupied: 0, price: 0, priceType: 'person' as const, amenities: [] as string[] }] }); }}
          ...
          <div className="space-y-10">
            <div className="space-y-6">
              <div>
                ...
                <p className="text-[10px] font-bold text-foreground/60 dark:text-muted-foreground uppercase tracking-widest">Select what your boarding house offers</p>
              </div>
                ...
                {commonAmenities.map(am => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-[11px] font-black uppercase tracking-tight transition-all ${formData.amenities.includes(am.id)
                        ...
                        : 'bg-background dark:bg-background/50 text-foreground/70 dark:text-muted-foreground border-border dark:border-border/50 hover:border-primary/50'
                      }`}
                  >
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-[10px] font-bold text-foreground/60 dark:text-muted-foreground uppercase tracking-widest">Guidelines for your tenants</p>
              </div>
              ...
              <div>
                <textarea
                  ...
                  className="w-full h-40 p-4 rounded-xl border border-border dark:border-border/60 bg-background focus:border-primary outline-none transition-all resize-none text-[13px] placeholder:text-foreground/40 dark:placeholder:text-muted-foreground/50 leading-relaxed shadow-inner"
                />
              </div>
            </div>
          </div>
      ...
      case 4:
        return (
          <div className="space-y-8">
            <div
              className="bg-primary/10 dark:bg-primary/5 border-2 border-dashed border-primary/20 rounded-[32px] p-12 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-primary/10 transition-all relative"
            ...
            >
              ...
              <p className="text-sm text-foreground/60 dark:text-muted-foreground">Select images of your property.</p>
            </div>

            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {images.map((img, i) => (
                <div key={i} className="aspect-square rounded-[24px] overflow-hidden bg-muted relative group border border-border dark:border-border/50 shadow-sm">
      ...
      case 5:
        return (
          <div className="space-y-8">
            <div className="bg-primary/10 dark:bg-primary/5 rounded-[32px] p-8 border border-primary/20 flex flex-col items-center justify-center text-center">
              <Check className="w-10 h-10 text-primary mb-4" />
              <h3 className="text-2xl font-black mb-2">Ready to Publish?</h3>
              <p className="text-sm text-foreground/70 dark:text-muted-foreground max-w-sm mx-auto">
                Once published, your listing will be immediately visible on the public map.
              </p>
            </div>

            <div className="bg-card rounded-[32px] border border-border/70 shadow-sm overflow-hidden flex flex-col">
              <div className="relative h-[240px] w-full bg-muted shrink-0">
                {images.length > 0 ? (
                ...
                <div className="absolute bottom-6 left-6 right-6 text-white text-left">
                  <h1 className="text-3xl font-black tracking-tighter drop-shadow-md">{formData.name || 'Untitled Property'}</h1>
                  <p className="flex items-center gap-1.5 text-sm font-medium mt-1 text-white/90">
                    <MapPin className="w-4 h-4" /> {formData.address || 'Address not set'}
                  </p>
                </div>
              </div>

              {/* Preview Details Body */}
              <div className="p-6 md:p-8 space-y-8 bg-card flex-1">
                
                {/* Description */}
                {(formData.description || formData.rules) && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {formData.description && (
                      <div>
                        <h4 className="text-[10px] font-black uppercase tracking-[0.15em] text-primary/80 dark:text-primary/60 mb-3">Description</h4>
                        <p className="text-[13px] font-medium leading-relaxed text-foreground whitespace-pre-wrap">{formData.description}</p>
                      </div>
                    )}
                    {formData.rules && (
                      <div>
                        <h4 className="text-[10px] font-black uppercase tracking-[0.15em] text-primary/80 dark:text-primary/60 mb-3">House Rules</h4>
                        <p className="text-[13px] font-medium leading-relaxed text-foreground whitespace-pre-wrap">{formData.rules}</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div>
                     <h4 className="text-[10px] font-black uppercase tracking-[0.15em] text-primary/80 dark:text-primary/60 mb-3 flex items-center gap-2">Contact Info</h4>
                     <div className="space-y-2.5">
                       <p className="flex items-center gap-2.5 text-[13px] font-bold text-foreground">
                         <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center"><Phone className="w-3 h-3 text-primary" /></div> 
                         {formData.phone || 'No phone set'}
                       </p>
                       <p className="flex items-center gap-2.5 text-[13px] font-bold text-foreground">
                         <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center"><Mail className="w-3 h-3 text-primary" /></div> 
                   ...
                   {formData.amenities.length > 0 && (
                     <div>
                       <h4 className="text-[10px] font-black uppercase tracking-[0.15em] text-primary/80 dark:text-primary/60 mb-3">Property Amenities</h4>
                       <div className="flex flex-wrap gap-2">
                         {formData.amenities.map(am => (
                           <span key={am} className="px-3 py-1.5 rounded-lg bg-muted text-[10px] font-black uppercase tracking-tight text-foreground border border-border dark:border-border/50">
                             {am}
                           </span>
                         ))}
                       </div>
                ...
                {formData.rooms.length > 0 && <div className="h-px bg-border/50" />}

                {/* Units Summary */}
                {formData.rooms.length > 0 && (
                  <div>
                     <h4 className="text-[10px] font-black uppercase tracking-[0.15em] text-primary/80 dark:text-primary/60 mb-4 flex items-center gap-2"><BedDouble className="w-3.5 h-3.5" /> Unit Inventory ({formData.rooms.length})</h4>
                     <div className="space-y-3">
                       {formData.rooms.map((room, idx) => (
                         <div key={idx} className="p-4 sm:p-5 rounded-2xl border border-border dark:border-border/50 bg-muted/80 dark:bg-muted/50 dark:bg-muted/20 flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-primary/40 transition-colors">
                            <div className="flex-1">
                               <p className="text-[14px] font-black text-foreground mb-2 leading-none">{room.name || 'Unnamed Unit'}</p>
                               <div className="flex flex-wrap items-center gap-2 text-muted-foreground">
                                 <span className="text-[9px] font-black uppercase tracking-[0.1em] text-foreground/70 bg-muted/80 dark:bg-muted/50 px-2 py-0.5 rounded border border-border dark:border-border/50">{room.type || 'No Type'}</span>
                                 <span className={`text-[9px] font-black uppercase tracking-[0.1em] px-2 py-0.5 rounded border border-border dark:border-border/50 flex items-center gap-1 ${
                                   room.gender === 'male' ? 'text-blue-500' : 
                                   room.gender === 'female' ? 'text-rose-500' : 'text-amber-500'
                                 }`}>
                                   {room.gender || 'mixed'}
                                 </span>
                                 <span className="text-[11px] font-bold flex items-center gap-1 ml-1 text-foreground/50">
                                    <Users className="w-3 h-3" /> {room.occupied} / {room.capacity}
                                 </span>
                               </div>
                               {room.amenities && room.amenities.length > 0 && (
                                  <p className="text-[10px] text-muted-foreground/80 font-bold mt-2.5 leading-relaxed bg-background dark:bg-background/50 inline-block px-2 py-1 rounded border border-border dark:border-border/50">
                                     <span className="opacity-60">Features:</span> {room.amenities.join(' • ')}
                                  </p>
                               )}
                            </div>
                            <div className="text-left md:text-right shrink-0 mt-2 md:mt-0 pb-1 md:pb-0">
                              <p className="text-xl font-black text-primary">₱{room.price?.toLocaleString() || 0}</p>
                              <p className="text-[9px] font-black uppercase tracking-[0.15em] text-muted-foreground mt-0.5">
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row bg-background">
      <div className="w-full lg:w-[320px] bg-background border-r border-border dark:border-border/50 shrink-0 flex flex-col sticky top-0 h-screen z-10 overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="p-6 pb-2">
            <Link to="/owner/properties" className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary transition-all mb-6 group">
              <div className="w-8 h-8 rounded-full bg-muted/80 dark:bg-muted/50 border border-border dark:border-border/50 flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground group-hover:border-primary transition-all duration-300">
                <ChevronLeft className="w-4 h-4" />
              </div>
              <span>Back to Properties</span>
            </Link>
            <h1 className="text-2xl font-black tracking-tight mb-1 select-none bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">New Listing</h1>
            <p className="text-muted-foreground/60 text-[12px] font-medium leading-relaxed select-none">Start your property journey</p>
          </div>

          <div className="px-6 mt-1 mb-4 select-none">
            <div className="h-1.5 w-full bg-muted/60 dark:bg-muted/30 rounded-full overflow-hidden shrink-0 mt-2">
              <motion.div
            ...
            <div className="flex justify-between items-center mt-2.5">
              ...
              <span className="text-[10px] font-black text-muted-foreground/70 dark:text-muted-foreground/40 tabular-nums">{progress}%</span>
            </div>
          </div>
        </div>
              ...
              return (
                <div
                  ...
                  className={`group w-full text-left p-3 rounded-[20px] flex items-center gap-4 transition-all duration-500 relative ${idx === currentStep
                    ...
                    } ${idx > currentStep ? 'opacity-70 grayscale-[30%]' : ''}`}
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <p className={`font-black text-[12px] uppercase tracking-wider transition-colors duration-300 ${idx === currentStep ? 'text-primary' : 'text-foreground/90'}`}>
                        {step.title}
                      </p>
                    </div>
                    <p className={`text-[10px] font-bold tracking-tight truncate mt-0.5 transition-colors duration-300 ${idx === currentStep ? 'text-primary/70' : 'text-muted-foreground/80'}`}>
                      {step.description}
                    </p>
                  </div>
                </div>
      ...
      <div className="flex-1 flex flex-col bg-background dark:bg-background/50 relative">
        ...
        <div className="sticky bottom-0 left-0 right-0 bg-background/80 backdrop-blur-xl border-t border-border dark:border-border/50 p-6 z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex gap-4">
              {currentStep > 0 && (
                <button
                  onClick={handleBack}
                  className="h-14 px-8 rounded-full border border-border dark:border-border/50 font-bold hover:bg-muted/80 dark:bg-muted/50 hover:border-primary/50 transition-all text-sm flex items-center gap-2"
                >
